...
function MethodologyView() {
  const qs = [
    ...
    { n: "9", t: "Why do the scores show a range and a confidence label?", d: "Complex systems carry real uncertainty, and a single deterministic number can imply a precision the model does not have. So every headline and sector score shows a ± sensitivity range and a confidence tier alongside the central value. The range is built by re-running the whole model with each editable assumption — the 0.60 exposure anchor, the 90-day Absorb benchmark, the sovereign-buffer sub-score, the consequence weights and the DRI scores — pushed across a plausible band, then combining the swings in quadrature. Open any score's explainer to see the ‘What moves it most’ tornado: the assumptions are ranked by how much each one widens the range, so you can see where the uncertainty comes from and which lever to pin down first. A tighter range earns a higher confidence label." },
        ...
        <div className="stack">
          ...
          <Panel title="Assumptions ledger" icon="book" label="EDITABLE">
            {[
              ...
              "Score uncertainty = quadrature of sensitivities to the editable assumptions (0.60 anchor, 90-day benchmark, sovereign buffer, consequence weights, DRI ±4 — which now spans the route weighting and 180-day buffer horizon); confidence tier follows the range width",
