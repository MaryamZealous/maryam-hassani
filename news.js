...
*/
const HEAD_OK = {   // cache real data at the CDN so repeat loads don't re-hit GDELT
...
async function gdeltOnce(q, max) {
  ...
  if (!r.ok) { const err = new Error(`gdelt ${r.status}`); err.status = r.status; throw err; }
...
// GDELT rate-limits to ~1 req/5s per IP and Netlify's shared egress IPs get
// throttled (HTTP 429) more often. Retry with backoff — a retry frequently
// lands on a fresh IP or after the throttle window clears. Kept well inside the
// function's ~10s timeout.
async function gdelt(q, max) {
  const backoffs = [0, 1800, 3600];
  let lastErr;
  for (const ms of backoffs) {
    if (ms) await wait(ms);
    try { return await gdeltOnce(q, max); }
...
exports.handler = async function () {
  try {
    const arts = await gdelt(COMBINED, 200);
    const areas = bucket(arts);
    return { statusCode: 200, headers: HEAD_OK, body: JSON.stringify({ ok: true, areas, ts: Date.now() }) };
  } catch (e) {
    // graceful: client falls back to its own attempt, then to simulation.
    // We NEVER fabricate zero articles — a failure is reported as ok:false and
    // is NOT cached, so the next load retries GDELT immediately.
    return { statusCode: 200, headers: HEAD_FAIL, body: JSON.stringify({ ok: false, error: String(e) }) };
  }
};
